...
PermissionHelper::requireAccess('settings');
                        ...
                        <div class="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
                            <div class="flex items-start">
                                ...
                                <div class="text-sm text-gray-700">
                                    <p class="font-semibold mb-1">Informasi Notifikasi:</p>
                                    <ul class="list-disc ml-5 space-y-1">
                                        <li><strong>Email Fallback:</strong> Notifikasi akan dikirim via email</li>
                                        <li><strong>Alert Otomatis:</strong> Notifikasi akan dikirim ketika AQI melebihi threshold (≥150)</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
...
z        // Notification Panel
    ...
    <script src="/siapkak/js/push-notifications.js"></script>
    ...
    <script>
        ...
        $('#test-notification-btn').on('click', function() {
            ...
            if ('Notification' in window && Notification.permission === 'granted') {
                // Send via backend to test both push and email
                $.ajax({
                    url: '/siapkak/api/notifications/test',
                    type: 'POST',
                    contentType: 'application/json',
                    xhrFields: { withCredentials: true },
                    success: function(response) {
                        ...
                        if (response.success) {
                            ...
                            alert('Failed to send test notification: ' + (response.message || 'Unknown error'));
                        }
                    },
                    error: function(xhr) {
                        btn.prop('disabled', false).html(originalText);
                ...
                if ('Notification' in window) {
                    Notification.requestPermission().then(permission => {
                        if (permission === 'granted') {
                            new Notification('SIAPKAK Test Notification', {
                                body: 'This is a test notification from SIAPKAK system.',
                                icon: '/siapkak/img/logo-siapkak.png',
                                badge: '/siapkak/img/logo-siapkak.png'
                            });
                        }
                        btn.prop('disabled', false).html(originalText);
                    });
        ...
        function loadNotificationHistory() {
            $.ajax({
                url: '/siapkak/api/notifications',
                type: 'GET',
                xhrFields: { withCredentials: true },
                success: function(response) {
                    if (response.success && response.data.notifications && response.data.notifications.length > 0) {
                        const history = response.data.notifications.slice(0, 10); // Last 10
                        let html = '';
                        
                        history.forEach(notif => {
                            const iconClass = notif.type === 'danger' ? 'fa-exclamation-circle text-red-500' :
                            ...
                            html += `
                                <div class="border-l-4 ${notif.type === 'danger' ? 'border-red-500' : notif.type === 'warning' ? 'border-yellow-500' : 'border-blue-500'} bg-gray-50 p-4 rounded-r">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="flex items-center gap-4 mt-2 text-xs text-gray-500">
                                                ...
                                                ${notif.delivery_method ? `<span><i class="fas fa-paper-plane mr-1"></i>${notif.delivery_method}</span>` : ''}
                                                ${notif.delivery_status ? `<span class="${notif.delivery_status === 'sent' ? 'text-green-600' : 'text-red-600'}"><i class="fas ${notif.delivery_status === 'sent' ? 'fa-check-circle' : 'fa-times-circle'} mr-1"></i>${notif.delivery_status}</span>` : ''}
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        // Load history when settings section is shown
        $(document).on('click', 'a[href="#settings"]', function() {
