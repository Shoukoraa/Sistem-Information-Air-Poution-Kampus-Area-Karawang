...
try {
    ...
    PermissionHelper::requireAccess('dashboard');
} catch (\Exception $e) {
    // Log error
    ...
    die("<!DOCTYPE html><html><head><title>Error</title><style>body{font-family:sans-serif;padding:50px;text-align:center;background:#f5f5f5;}h1{color:#dc2626;}</style></head><body><h1>⚠️ Dashboard Error</h1><p>Maaf, terjadi kesalahan saat memuat dashboard.</p><p>Silakan <a href='/siapkak/debug.php'>cek diagnostic tool</a> atau hubungi administrator.</p><p><a href='/siapkak/dashboard-test.php'>Coba dashboard simple</a> | <a href='/siapkak/auth/logout.php'>Logout</a></p></body></html>");
}

// Get user role
$userRole = $_SESSION['user_role'] ?? null;
$showFavoritesWidget = ($userRole === 'user');
...
<body class="bg-gray-100">
    ...
    <div class="flex pt-16">
        ...
        <main class="flex-1 min-h-[calc(100vh-64px)] p-4 sm:p-6 lg:p-8">
            ...
            <section id="overview" class="content-section">
                ...
                <!-- Analytics & Insights dalam Overview -->
                <div class="mt-8">
                    <h3 class="text-xl sm:text-2xl font-bold text-gray-900 mb-6">Analytics & Insights</h3>
                    
                    <!-- My Favorite Stations Widget (Only for user role) -->
                    <?php if ($showFavoritesWidget): ?>
                    <div id="favoritesWidget" class="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl shadow-sm p-6 mb-6">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-lg font-semibold text-gray-900">
                                <i class="fas fa-star mr-2 text-yellow-500"></i>
                                My Favorite Stations
                            </h4>
                            <a href="/siapkak/stations" class="text-sm text-blue-600 hover:text-blue-800">
                    ...
                    <!-- Station AQI Comparison Chart -->
                    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="text-lg font-semibold text-gray-900">
                                <i class="fas fa-chart-bar mr-2 text-blue-600"></i>
                                Perbandingan AQI Per Stasiun
                            </h4>
                            <div class="flex gap-2">
                                <span class="px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                                    <i class="fas fa-broadcast-tower mr-1"></i>Real-time Data
                                </span>
                            </div>
                        </div>
                        <div class="h-80">
                        ...
                        <p class="text-xs text-gray-500 mt-2">
                            ...
                            Grafik menampilkan nilai AQI terkini dari setiap stasiun monitoring
                        </p>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div id="airQualityMap" class="rounded-lg border-2 border-gray-200" style="height: 600px; min-height: 600px;"></div>
                        ...
                        <div class="mt-4 p-4 bg-blue-50 border-l-4 border-blue-500 rounded">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <h5 class="text-sm font-semibold text-gray-800 mb-2">Data AQI Aktif di Peta:</h5>
            ...
            <!-- Readings Section -->
            <section id="readings" class="content-section hidden">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
                    <div>
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ID</th>
                                    <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Stasiun</th>
                                    <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">AQI</th>
                                    <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">PM2.5</th>
                                    <th class="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">PM10</th>
                                    <th class="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">O₃</th>
                                    <th class="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">NO₂</th>
                                    <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Waktu</th>
    ...
    $(document).ready(function() {
        ...
        // Initialize Charts (chart init can happen early)
        initCharts();
        
        // Initialize Leaflet Map (map is in overview section, visible by default)
        setTimeout(function() {
            initLeafletMap();
        }, 500);
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Load overview data only after auth so session is valid
                    loadOverviewData();
                } else {
                    redirectToLogin();
                }
    ...
    function submitPasswordChange() {
        ...
        $.ajax({
            url: '/siapkak/auth/change-password.php',
            type: 'POST',
    ...
    function syncStationsSequentially(stations, index, btn, originalHtml) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Continue to next station with longer delay to avoid rate limiting
                setTimeout(function() {
                    syncStationsSequentially(stations, index + 1, btn, originalHtml);
                }, 2000); // 2 second delay between requests to respect API rate limits
            },
            error: function(xhr) {
                const errorMsg = xhr.responseJSON?.message || 'API error';
                console.warn(`⚠️ Skipped ${station.name}: ${errorMsg}`);
                // Continue to next station with longer delay on error
                setTimeout(function() {
                    syncStationsSequentially(stations, index + 1, btn, originalHtml);
                }, 2000); // 2 second delay even on error
            }
        });
    }
    ...
    // ============================================
    let stationComparisonChart, pollutantsChart, aqiPieChart;
    ...
    function loadOverviewData() {
        loadOverviewStats();
        loadOverviewCharts();
        // Only load favorites if widget is visible (user role only)
        if ($('#favoritesWidget').length > 0) {
    ...
    function loadFavoriteStations() {
        const renderEmpty = () => {
            $('#favoritesContent').html(`
                <div class="text-center py-6 text-gray-500">
                    ...
                    <p class="text-xs mt-1">Go to <a href="/siapkak/stations" class="text-blue-600 hover:underline">Stations</a> page to add favorites</p>
                </div>
            `);
        };
        ...
        const renderError = (msg) => {
            $('#favoritesContent').html(`
                <div class="text-center py-6 text-gray-500">
                    ...
                    <p class="text-sm">${msg || 'Failed to load favorites'}</p>
                </div>
            `);
        };
        ...
        const renderList = (favorites) => {
            ...
            favorites.forEach(fav => {
                const aqiClass = getAqiClass(fav.aqi);
                const aqiLabel = fav.aqi ? `AQI: ${fav.aqi}` : 'No Data';
                const pm25 = fav.pm25 ? `${fav.pm25} µg/m³` : '-';
                const stationName = fav.station_name || fav.name || `Station #${fav.station_id}`;
                const stationLocation = fav.location || 'Unknown location';

                html += `
                    <div class="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition cursor-pointer" onclick="viewFavoriteStationDetail(${fav.station_id})">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <h5 class="font-semibold text-gray-900">${escapeHtml(stationName)}</h5>
                                <p class="text-xs text-gray-500 mt-1">
                                    <i class="fas fa-map-marker-alt mr-1"></i>${escapeHtml(stationLocation)}
                                </p>
                            </div>
                            <span class="px-2 py-1 text-xs rounded-full ${aqiClass} font-semibold ml-2">
                                ${aqiLabel}
                            </span>
                        </div>
                        <div class="mt-3 flex items-center justify-between text-xs text-gray-600">
                            <span><i class="fas fa-smog mr-1"></i>PM2.5: ${pm25}</span>
                            ${fav.recorded_at ? `<span class="text-gray-400">${formatTimeAgo(fav.recorded_at)}</span>` : ''}
                        </div>
                    </div>
                `;
            });
        ...
        const fetchFallback = () => {
            $.ajax({
                ...
                error: function() {
                    renderError('Failed to load favorites');
                }
            });
        };

        $.ajax({
            url: '/siapkak/api/favorites/with-readings?limit=5',
            type: 'GET',
            xhrFields: { withCredentials: true },
            success: function(response) {
                if (response.success && response.data.favorites) {
                    renderList(response.data.favorites);
                } else {
                    fetchFallback();
                }
            },
    ...
    // ============================================
    function initCharts() {
        // Station Comparison Chart (Horizontal Bar Chart)
        const scCtx = document.getElementById('stationComparisonChart');
        if (scCtx) {
            loadStationComparisonData();
        }
    ...
    function loadStationComparisonData() {
        $.ajax({
            url: '/siapkak/api/readings?limit=100',
            type: 'GET',
            xhrFields: { withCredentials: true },
            success: function(response) {
                if (response.success && response.data && response.data.readings && response.data.readings.length > 0) {
                    const readings = response.data.readings;
                    
                    // Group by station and get latest reading per station
                    const stationMap = {};
                    readings.forEach(r => {
                        if (r.station_name && r.aqi_index !== null && r.aqi_index !== undefined) {
                            // Only keep latest reading per station
                            if (!stationMap[r.station_name] || 
                                new Date(r.measured_at) > new Date(stationMap[r.station_name].measured_at)) {
                                stationMap[r.station_name] = {
                                    station_name: r.station_name,
                                    aqi_index: parseFloat(r.aqi_index),
                    ...
                    // Convert to array and sort by AQI descending
                    const stationsData = Object.values(stationMap);
                    stationsData.sort((a, b) => b.aqi_index - a.aqi_index);
                    
                    // Take top 10 stations
                    const top10 = stationsData.slice(0, 10);
                    
                    if (top10.length === 0) {
                        showEmptyChart('stationComparisonChart', 'No data available');
                        return;
                    }
                    
                    const labels = top10.map(s => s.station_name);
                    const aqiValues = top10.map(s => s.aqi_index);
                    
                    // Color based on AQI level
                    const backgroundColors = aqiValues.map(aqi => {
                        if (aqi <= 50) return 'rgba(34, 197, 94, 0.8)';      // Green - Baik
                        if (aqi <= 100) return 'rgba(234, 179, 8, 0.8)';     // Yellow - Sedang
                        if (aqi <= 150) return 'rgba(249, 115, 22, 0.8)';    // Orange - Sensitif
                        if (aqi <= 200) return 'rgba(239, 68, 68, 0.8)';     // Red - Tidak Sehat
                        if (aqi <= 300) return 'rgba(168, 85, 247, 0.8)';    // Purple - Sangat Buruk
                        return 'rgba(127, 29, 29, 0.8)';                      // Maroon - Berbahaya
                    });
                    
                    const borderColors = backgroundColors.map(color => color.replace('0.8', '1'));
                    
                    const ctx = document.getElementById('stationComparisonChart');
                    if (ctx) {
                        stationComparisonChart = new Chart(ctx.getContext('2d'), {
                            type: 'bar',
                            data: {
                                labels: labels,
                                datasets: [{
                            ...
                            options: {
                                ...
                                plugins: {
                                    ...
                                    tooltip: {
                                        callbacks: {
                                            label: function(context) {
                                                const aqi = context.parsed.x;
                                                let status = '';
                                                if (aqi <= 50) status = 'Baik';
                                                else if (aqi <= 100) status = 'Sedang';
                                                else if (aqi <= 150) status = 'Tidak Sehat (Sensitif)';
                                                else if (aqi <= 200) status = 'Tidak Sehat';
                                                else if (aqi <= 300) status = 'Sangat Tidak Sehat';
                                                else status = 'Berbahaya';
                                                
                                                return 'AQI: ' + aqi.toFixed(0) + ' (' + status + ')';
                                            }
                                        }
                                    }
                                },
                        ...
                        console.log('✅ Station comparison chart loaded with', top10.length, 'stations');
                    }
                } else {
                    showEmptyChart('stationComparisonChart', 'No data available');
                }
            ...
            error: function(xhr, status, error) {
                console.error('Failed to load station comparison data:', error);
                showEmptyChart('stationComparisonChart', 'Error loading data');
            }
        });
    }
    ...
    function initLeafletMap() {
        ...
        setTimeout(function() {
            try {
                ...
                airQualityMap = L.map('airQualityMap').setView([-2.5, 118], 5);
    ...
    // View favorite station detail - fetch full data
    function viewFavoriteStationDetail(stationId) {
        // Show loading state
        const modalContent = `
            <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" id="favoriteDetailModal">
                <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    <div class="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
                        <h3 class="text-xl font-bold text-gray-900">Station Details</h3>
                        <button onclick="closeFavoriteDetailModal()" class="text-gray-400 hover:text-gray-600">
                            <i class="fas fa-times text-xl"></i>
                        </button>
                    </div>
                    <div class="p-6" id="favoriteDetailContent">
                        <div class="text-center py-8">
                            <i class="fas fa-spinner fa-spin text-3xl text-blue-500"></i>
                            <p class="mt-2 text-gray-600">Loading station data...</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $.ajax({
            url: `/siapkak/api/stations/show?id=${stationId}`,
            type: 'GET',
            xhrFields: { withCredentials: true },
            success: function(response) {
                if (response.success && response.data) {
                    const stationData = {
                        ...
                        ...response.data.latest_reading
                    };
                    renderFavoriteDetailContent(stationData);
                } else {
                    showDetailError('Station data not found');
                }
            },
            error: function(xhr) {
                const errorMsg = xhr.responseJSON?.message || 'Failed to load station details';
                showDetailError(errorMsg);
            }
        });
    }

    function renderFavoriteDetailContent(station) {
        const aqiClass = getAqiClass(station.aqi);
        const aqiLabel = station.aqi || 'N/A';
        const stationName = station.name || station.station_name || `Station #${station.id}`;
        const location = station.location || 'Unknown location';
        
        const content = `
            <div class="space-y-4">
                <!-- Header with AQI -->
                <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-6">
                    <h4 class="text-2xl font-bold text-gray-900 mb-2">${escapeHtml(stationName)}</h4>
                    <p class="text-sm text-gray-600 mb-4">
                        <i class="fas fa-map-marker-alt mr-1"></i>${escapeHtml(location)}
                    </p>
                    <div class="flex items-center gap-4">
                        <div class="text-center">
                            <div class="text-4xl font-bold ${aqiClass}">${aqiLabel}</div>
                            <div class="text-sm text-gray-600 mt-1">AQI</div>
                        </div>
                        ${station.aqi_category ? `
                        <div class="text-sm text-gray-700">
                            <div class="font-semibold">${station.aqi_category}</div>
                            <div class="text-xs">${getAqiDescription(station.aqi)}</div>
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ${renderPollutantCard('PM2.5', station.pm25, 'µg/m³', 'fa-smog')}
                    ${renderPollutantCard('PM10', station.pm10, 'µg/m³', 'fa-wind')}
                    ${renderPollutantCard('Temperature', station.temperature, '°C', 'fa-temperature-high')}
                    ${renderPollutantCard('Humidity', station.humidity, '%', 'fa-tint')}
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <p class="text-sm text-gray-600">${escapeHtml(station.description)}</p>
                </div>
                ` : ''}
                ...
                <div class="text-center text-xs text-gray-500 pt-2 border-t">
                    ...
                    Last updated: ${formatTimeAgo(station.recorded_at)}
                </div>
                ` : ''}
            </div>
        `;
        
        $('#favoriteDetailContent').html(content);
    }

    function renderPollutantCard(label, value, unit, icon) {
        const displayValue = value !== null && value !== undefined ? `${value} ${unit}` : 'N/A';
        return `
            <div class="bg-white border rounded-lg p-4 text-center">
                <i class="fas ${icon} text-2xl text-blue-500 mb-2"></i>
    ...
    function showDetailError(message) {
        $('#favoriteDetailContent').html(`
            <div class="text-center py-8">
                <i class="fas fa-exclamation-circle text-4xl text-red-400 mb-3"></i>
                <p class="text-gray-600">${message}</p>
                <button onclick="closeFavoriteDetailModal()" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                    Close
                </button>
            </div>
        `);
        ...
        readings.forEach(function(reading) {
            ...
            html += `
                <tr class="hover:bg-gray-50" data-station-id="${reading.station_id}">
                    <td class="hidden md:table-cell px-6 py-4 text-sm text-gray-900">${reading.id}</td>
                    <td class="px-4 md:px-6 py-4 text-sm text-gray-600">${escapeHtml(reading.station_name || '-')}</td>
                    <td class="px-4 md:px-6 py-4 text-sm">
                        <span class="px-2 py-1 text-xs rounded-full ${aqiClass} font-semibold">
                            ${reading.aqi_index || '-'}
                        </span>
                    </td>
                    <td class="px-4 md:px-6 py-4 text-sm text-gray-600">${formatNumber(reading.pm25)}</td>
                    <td class="hidden lg:table-cell px-6 py-4 text-sm text-gray-600">${formatNumber(reading.pm10)}</td>
                    <td class="hidden lg:table-cell px-6 py-4 text-sm text-gray-600">${formatNumber(reading.o3)}</td>
                    <td class="hidden lg:table-cell px-6 py-4 text-sm text-gray-600">${formatNumber(reading.no2)}</td>
                    <td class="hidden md:table-cell px-6 py-4 text-sm text-gray-600">${formatDateTime(reading.measured_at)}</td>
                    <td class="px-4 md:px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
    ...
    function saveReading() {
        ...
        const data = {
            ...
            recorded_at: $('#readingRecordedAt').val() || null
        };
    ...
    window.viewReading = function(id) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    const reading = response.data;
                    const aqiClass = getAqiClass(reading.aqi);
                    $('#viewReadingContent').html(`
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="text-sm font-medium text-gray-500">ID</label>
                                    <p class="text-lg">${reading.id}</p>
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <p class="text-lg">${formatDateTime(reading.recorded_at)}</p>
                            </div>
                        </div>
                    `);
