...
PermissionHelper::requireAccess('stations');

// Get user role for features visibility
$userRole = $_SESSION['user_role'] ?? null;
$showFavoritesFeature = ($userRole === 'user');
...
<body class="bg-gray-100">
    ...
    <div class="flex pt-16">
        ...
        <main class="flex-1 min-h-[calc(100vh-64px)] p-4 sm:p-6 lg:p-8">
            ...
            <section id="overview" class="content-section hidden">
                <div class="flex flex-col sm:flex-row sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
    ...
    // GLOBAL VARIABLES
    // ============================================
    let userFavoriteIds = []; // Store user's favorite station IDs

    $(document).ready(function() {
        // Check authentication first
        checkAuth();
        
        // Load user favorites (only if user role)
        <?php if ($showFavoritesFeature): ?>
        loadUserFavorites();
        <?php endif; ?>
    ...
    function syncAllStationsData() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                console.error('Failed to fetch stations:', xhr.responseText);
            }
        });
    }
    ...
    function syncStationsSequentially(stations, index, btn, originalHtml) {
        if (index >= stations.length) {
            ...
            console.log('=== ALL STATIONS SYNCED ===');
            console.log(`Total stations: ${stations.length}`);
            console.log('Check your email: laporsiapkak@gmail.com');
        ...
        console.log(`=== SYNCING STATION ${progress} ===`);
        ...
        $.ajax({
            ...
            success: function(response) {
                console.log(`✅ SUCCESS: ${station.name}`);
                console.log('AQI:', response.data?.aqi || 'N/A');
                console.log('Status:', response.data?.status || 'N/A');
                console.log('Notification sent:', response.data?.notification_sent || false);
    ...
    function initStationsCRUD() {
        ...
        // If stations section is already visible on load, load data immediately
        if (!$('#stations').hasClass('hidden')) {
            loadStationsForCRUD();
        }

        // Ensure stations are loaded when navigating to the stations section
        // Support multiple selector patterns to match the markup used in the app
        $('.nav-link[data-target="stations"], a[href="#stations"], #navStations').on('click', function() {
    ...
    function displayStationsTable(stations) {
        ...
        stations.forEach(function(station) {
            ...
            const isFavorite = userFavoriteIds.includes(station.id);
            const favoriteBtn = <?php echo $showFavoritesFeature ? 'true' : 'false'; ?> ? (isFavorite ?
                `<button onclick="toggleFavorite(${station.id})" class="text-yellow-500 hover:text-yellow-600" title="Remove from Favorites">
                    <i class="fas fa-star"></i>
                </button>` :
                `<button onclick="toggleFavorite(${station.id})" class="text-gray-400 hover:text-yellow-500" title="Add to Favorites">
    ...
    function openStationModal(isEdit) {
        <?php if (!PermissionHelper::isAdmin()): ?>
        alert('Anda tidak memiliki akses untuk menambah/mengedit data stasiun.');
        return;
        <?php endif; ?>
    ...
    window.editStation = function(id) {
        <?php if (!PermissionHelper::isAdmin()): ?>
        alert('Anda tidak memiliki akses untuk mengedit data stasiun.');
        return;
        <?php endif; ?>

        $.ajax({
            url: '/siapkak/api/stations/show?id=' + id,
            type: 'GET',
            xhrFields: { withCredentials: true },
            success: function(response) {
                if (response.success) {
    ...
    window.deleteStation = function(id, name) {
        <?php if (!PermissionHelper::isAdmin()): ?>
        alert('Anda tidak memiliki akses untuk menghapus data stasiun.');
        return;
        <?php endif; ?>
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-lg font-semibold">${station.id}</p>
                                </div>
                                <div>
                                    <label class="text-sm font-medium text-gray-500">External ID</label>
                                    <p class="text-lg">${station.external_id || '-'}</p>
                                </div>
                            </div>
                            <div>
                            ...
                            <div class="grid grid-cols-2 gap-4 border-t pt-4">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">${formatDateTime(station.created_at)}</p>
                                </div>
                                <div>
                                    <label class="text-sm font-medium text-gray-500">Diperbarui Pada</label>
                                    <p class="text-sm text-gray-600">${formatDateTime(station.updated_at)}</p>
                                </div>
                            </div>
                        </div>
                    `);
    ...
    function loadUserFavorites() {
        $.ajax({
            url: '/siapkak/api/favorites/ids',
            type: 'GET',
            xhrFields: { withCredentials: true },
            success: function(response) {
                if (response.success && response.data.station_ids) {
                    userFavoriteIds = response.data.station_ids;
                }
            },
            error: function(xhr) {
                console.error('Failed to load favorites:', xhr.responseJSON);
            }
        });
    }

    window.toggleFavorite = function(stationId) {
        $.ajax({
            url: '/siapkak/api/favorites/toggle',
            type: 'POST',
            contentType: 'application/json',
            xhrFields: { withCredentials: true },
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.data.is_favorite) {
                        userFavoriteIds.push(stationId);
                        showAlert('success', 'Station added to favorites!');
                    } else {
                        userFavoriteIds = userFavoriteIds.filter(id => id !== stationId);
                        showAlert('success', 'Station removed from favorites!');
                    }
                    
                    // Reload stations to update UI
                    loadStations();
                } else {
                    showAlert('error', response.message || 'Failed to toggle favorite');
                }
            },
            error: function(xhr) {
                const errorMsg = xhr.responseJSON?.message || 'Failed to toggle favorite';
